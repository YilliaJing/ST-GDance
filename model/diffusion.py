...
class GaussianDiffusion(nn.Module):
    ...
    def long_ddim_sample(self, shape, cond, **kwargs):
        ...
        shape = 2, 3, 400, 151  # bn, num_persons, frames=150, hidden_dim   # 改这里
        x = torch.randn(shape, device = device)
        cond = cond.to(device)
    ...
    def p_losses(self, x_start, cond, t):
        ...
        model_contact, model_out = torch.split(
            model_out, (4, model_out.shape[-1] - 4), dim=-1
        )
        target_contact, target = torch.split(target, (4, target.shape[-1] - 4), dim=-1)
        ...
        # FK loss
        b, n_persons, s, c = model_out.shape
        # unnormalize
        # model_out = self.normalizer.unnormalize(model_out)
        # target = self.normalizer.unnormalize(target)
        # X, Q

        model_x = model_out[..., :3].reshape(b * n_persons, s, -1)
        model_q = ax_from_6v(model_out[..., 3:].reshape(b * n_persons, s, -1, 6))
        target_x = target[..., :3].reshape(b * n_persons, s, -1)
        target_q = ax_from_6v(target[..., 3:].reshape(b * n_persons, s, -1, 6))
        ...
        fk_loss = fk_loss.reshape(b, n_persons, -1).mean(1)
        ...
        static_idx = static_idx.reshape(b*n_persons, s, -1)
        ...
        foot_loss = foot_loss.reshape(b, n_persons, -1).mean(1)
        losses = (
            0.636 * loss.mean().float(),
            2.964 * v_loss.mean().float(),
            0.646 * fk_loss.mean().float(),
            10.942 * foot_loss.mean().float(),
        )
        return sum(losses).float(), losses
    ...
    def render_sample_multi(
        ...
        samples = normalizer.unnormalize(samples)  # Unnormalize samples
        if samples.shape[-1] == 151:
            sample_contact, samples = torch.split(
                samples, (4, samples.shape[-1] - 4), dim=-1
            )
        else:
            sample_contact = None

        b, max_persons, s, c = samples.shape  # 147
        pos = samples[:, :, :, :3].to(cond.device) # 3
        q = samples[:, :, :, 3:].reshape(b * max_persons, s, 24, 6)
        q = ax_from_6v(q).reshape(b, max_persons, s, -1, 3).to(cond.device)

        def merge_long_sequences(pos, q):
            b, s, c1, c2 = q.shape
            assert s % 2 == 0
            half = s // 2
            if b > 1:
            ...
            return full_pos, full_q

        b, max_persons, s, _ = pos.shape

        # 用 list 存放每个人拼好后的 pos, q
        merged_pos_list = []
        merged_q_list = []
        full_pose_list = []

        for person_idx in range(max_persons):
            # 取出该人的序列 => shape = [b, s, 3] / [b, s, 24, 3]
            pos_i = pos[:, person_idx]  # (b, s, 3)
            q_i = q[:, person_idx]  # (b, s, 24, 3)

            full_pos_i, full_q_i = merge_long_sequences(pos_i, q_i)
            full_pose_i = (
                self.smpl.forward(full_q_i, full_pos_i).detach().cpu()
            )  # b, s, 24, 3

            # 存起来
            merged_pos_list.append(full_pos_i)
            merged_q_list.append(full_q_i)
            full_pose_list.append(full_pose_i)

        # 把列表在第0维堆叠 => [max_persons, merged_length, ...]
        full_pos = torch.stack(merged_pos_list, dim=0)
        full_q = torch.stack(merged_q_list, dim=0)
        full_pose = torch.stack(full_pose_list, dim=0)
        ...
        if fk_out is not None:
            ...
            pickle.dump(
                {
                    "smpl_poses": full_q.squeeze(1).reshape((5, -1, 72)).cpu().numpy(),
                    "smpl_trans": full_pos.squeeze(1).cpu().numpy(),
                    "full_pose": full_pose[:, 0],
                },
                open(os.path.join(fk_out, outname), "wb"),
            )
