...
def test(opt):
    ...
    sample_size = 2 # int(sample_length / 2.5) - 1
    ...
    if opt.use_cached_features:
        ...
        for wav_file in glob.glob(os.path.join(opt.music_dir, "*.wav")):
            ...
            if len(file_list) < sample_size:
                print(
                    f"Skipping file {wav_file}: insufficient slices ({len(file_list)} available, need {sample_size}).")
                continue
    ...
    model = EDGE(opt.feature_type, opt.checkpoint, opt.learning_rate)
    model.eval()
    ...
    '''for i in range(len(all_cond)):
        ...
        print(f"Motions will be saved to: {fk_out}")
        model.render_sample_multi(
